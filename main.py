...
# Prepare containers for separate caches
...
original_training_match_images = []
        ...
        for strength in range(min_degradation_strength, max_degradation_strength):
            ...
            if degraded_enc is None:
                ...
                if save_degraded_images:
                    save_image(degraded_img, "output/no_embedding", f"img_{i}_{degradation_fn.__name__}_s{strength}.png", detector, embedder, device)
                continue
            ...
            if similarity > verification_threshold:
                ...
                if save_degraded_images:
                    save_image(degraded_img, "output/recognizable", f"img_{i}_{degradation_fn.__name__}_s{strength}_sim{similarity:.2f}.png", detector, embedder, device)
            else:
                ...
                if save_degraded_images:
                    save_image(degraded_img, "output/unrecognizable", f"img_{i}_{degradation_fn.__name__}_s{strength}_sim{similarity:.2f}.png", detector, embedder, device)

            training_match_total_count += 1

print("\nTraining match set statistics:")
print("Total images processed:", training_match_total_count)
...
for img, enc, _, _ in unrecognizable_training_match_images:
    if enc is not None:
...
for _, degraded_enc, _, _ in recognizable_training_match_images:
    X_train.append(degraded_enc)
    y_train.append(1)

# Label: 0 for unrecognizable
for _, degraded_enc, _, _ in unrecognizable_training_match_images:
    X_train.append(degraded_enc)
    y_train.append(0)
...
# Recognizable
for (_, degraded_enc, _, _), cache_key in zip(recognizable_training_match_images, embedding_cache.keys()):
    for fn in degradation_pool:
...
# Unrecognizable
for (_, degraded_enc, _, _), cache_key in zip(unrecognizable_training_match_images, embedding_cache.keys()):
    for fn in degradation_pool:
        if f"_{fn.__name__}_" in cache_key:
            per_degradation_data[fn.__name__]["X"].append(degraded_enc)
